...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['delete'])) {
        $id = $_POST['delete'];
        $sql = "DELETE FROM kiosk WHERE vendor_id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(':id', $id);
        if ($stmt->execute()) {
            $sql = "DELETE FROM foodvendor WHERE id = :id";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':id', $id);
            $stmt->execute();
        }
    ...
    <body>
        ...
        <script>
            ...
            <form action="admin_delete_vendor.php" method="post">
                <span>
                    <button type="submit" name="delete" value="${res[i].id}" class="btn btn-primary">Delete</button>
                </span>
            </form>
                </td>
            </tr>
